    ...
    class Users extends Model
    {
		...
		public function update(UserEntity $user) :array
		{
			try
			{
				...
				$stmt->bindValue(':updated_by', $user->updated_by(), \PDO::PARAM_INT);
				if(!empty($password)) {$stmt->bindValue(':password', $password,  \PDO::PARAM_STR);}
				$stmt->bindValue(':id', $user->id(), \PDO::PARAM_INT);
				$stmt->execute();

				// Update users_fields table
				$sql = "UPDATE
							users_fields
						SET
							gender = :gender,
				...
				$stmt  = $this->_db->prepare($sql);
				$stmt->bindValue(':gender', $user->gender(), \PDO::PARAM_INT);
				$stmt->bindValue(':description', $user->description(), \PDO::PARAM_STR);
				$stmt->bindValue(':birthdate', $user->birthdate() == '' ? null : $user->birthdate(), \PDO::PARAM_STR);
		...
		public function insert(UserEntity $user) :array
		{
			try
			{
				...
				$stmt->bindValue(':in_backend', $user->in_backend(), \PDO::PARAM_INT);
				$stmt->bindValue(':is_enabled', $user->is_enabled(), \PDO::PARAM_INT);
				$stmt->bindValue(':created_by', $user->updated_by(), \PDO::PARAM_INT);
				$stmt->bindValue(':updated_by', $user->updated_by(), \PDO::PARAM_INT);
				$stmt->execute();
		...
		private function __getUserEntity(int $id) :UserEntity
		{
			try
			{
				$sql = "SELECT
						...
						FROM
							users AS a
						LEFT JOIN users_fields AS b ON b.user_id = a.id
                        WHERE
                          a.id = :id
				";
